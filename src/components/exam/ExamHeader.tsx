    ...
    <header className="sticky top-0 z-40 border-b border-line bg-paper/95 px-4 py-3 backdrop-blur sm:px-6">
      <div className="mx-auto flex max-w-[1400px] items-center justify-between gap-4">
        <div>
          ...
          <p className="font-mono text-[10px] uppercase tracking-widest text-soft">{studentName} · Q {currentQuestion}/{totalQuestions}</p>
        </div>

        <div className="flex items-center gap-2 sm:gap-3">
          <div className="flex items-center gap-1.5 px-2">
            {autosaveStatus === "saving" && <span className="h-1.5 w-1.5 animate-pulse rounded-none bg-amber" />}
            {autosaveStatus === "saved" && <span className="h-1.5 w-1.5 rounded-none bg-success" />}
            {autosaveStatus === "local" && <span className="h-1.5 w-1.5 rounded-none bg-alert" />}
            {autosaveStatus === "failed" && <span className="h-1.5 w-1.5 rounded-none bg-alert" />}
          ...
          <button
            ...
            className="border border-line px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider text-ink"
