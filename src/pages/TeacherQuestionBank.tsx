...
export default function TeacherQuestionBank({ availableExams, navigate, notify }: { availableExams: Exam[]; navigate: (path: string) => void; notify: (message: string) => void }) {
  ...
  const urlExamId = searchParams.get("q") || "";
  const [selectedExamId, setSelectedExamId] = useState(urlExamId);
  const [activeTab, setActiveTab] = useState<"bank" | "pool" | "security">("bank");

  useEffect(() => {
    if (urlExamId && urlExamId !== selectedExamId) setSelectedExamId(urlExamId);
  }, [urlExamId]);

  useEffect(() => {
    if (selectedExamId) {
      const next = new URLSearchParams(searchParams);
      if (next.get("q") !== selectedExamId) {
        next.set("q", selectedExamId);
...
function SecurityPanel({ selectedExamId, notify }: { selectedExamId: string; notify: (s: string) => void }) {
  ...
  const [passM, setPassM] = useState(4);
  const [saved, setSaved] = useState(false);
  const save = () => { setSaved(true); notify("Security settings saved."); };
  if (!selectedExamId) return <div className="flex flex-col items-center p-12 text-center"><p className="font-serif text-xl">No exam selected</p><p className="mt-2 text-[12px] text-ink-soft">Create or select an exam above.</p></div>;
  return <div className="grid gap-6 p-6"><div className="grid gap-6 lg:grid-cols-2"><div className="border border-line p-5"><p className="font-mono text-[10px] uppercase tracking-widest text-ink-soft">Proctoring</p><div className="mt-4 space-y-3"><label className="flex items-center gap-3"><input type="checkbox" checked={ai} onChange={(e) => setAi(e.target.checked)} className="accent-forest"/><div><p className="text-[13px] font-medium">AI Proctoring</p><p className="text-[11px] text-ink-soft">Face & audio anomaly detection</p></div></label><label className="flex items-center gap-3"><input type="checkbox" checked={live} onChange={(e) => setLive(e.target.checked)} className="accent-forest"/><div><p className="text-[13px] font-medium">Live Teacher Proctoring</p><p className="text-[11px] text-ink-soft">Real-time monitoring from Proctor Dashboard</p></div></label></div></div><div className="border border-line p-5"><p className="font-mono text-[10px] uppercase tracking-widest text-ink-soft">Security</p><div className="mt-4 space-y-3"><label className="flex items-center gap-3"><input type="checkbox" checked={lock} onChange={(e) => setLock(e.target.checked)} className="accent-forest"/><div><p className="text-[13px] font-medium">Browser Lockdown</p><p className="text-[11px] text-ink-soft">Full-screen, no right-click</p></div></label><label className="flex items-center gap-3"><input type="checkbox" checked={noCopy} onChange={(e) => setNoCopy(e.target.checked)} className="accent-forest"/><div><p className="text-[13px] font-medium">Prevent Copy-Paste</p><p className="text-[11px] text-ink-soft">Block clipboard access</p></div></label></div></div></div><div className="border border-line p-5"><p className="font-mono text-[10px] uppercase tracking-widest text-ink-soft">Questions &amp; Marking</p><div className="mt-4 grid gap-4 sm:grid-cols-3"><div><label className="block text-[12px] text-ink-soft">Questions per student</label><input type="number" min="1" max="50" value={qps} onChange={(e) => setQps(parseInt(e.target.value) || 1)} className="mt-1 block w-full border border-line-strong bg-paper px-3 py-2.5 text-[13px] outline-none focus:border-forest"/></div><div><label className="block text-[12px] text-ink-soft">Total marks</label><input type="number" min="1" max="100" value={maxM} onChange={(e) => setMaxM(parseInt(e.target.value) || 1)} className="mt-1 block w-full border border-line-strong bg-paper px-3 py-2.5 text-[13px] outline-none focus:border-forest"/></div><div><label className="block text-[12px] text-ink-soft">Passing marks</label><input type="number" min="1" max={maxM} value={passM} onChange={(e) => setPassM(parseInt(e.target.value) || 1)} className="mt-1 block w-full border border-line-strong bg-paper px-3 py-2.5 text-[13px] outline-none focus:border-forest"/></div></div></div><div className="border border-line p-5"><p className="font-mono text-[10px] uppercase tracking-widest text-ink-soft">Question Randomization</p><div className="mt-4 flex flex-wrap gap-4"><label className="flex items-center gap-2"><input type="checkbox" checked={shuffleQ} onChange={(e) => setShuffleQ(e.target.checked)} className="accent-forest"/><span className="text-[13px]">Shuffle Questions</span></label><label className="flex items-center gap-2"><input type="checkbox" checked={shuffleO} onChange={(e) => setShuffleO(e.target.checked)} className="accent-forest"/><span className="text-[13px]">Shuffle MCQ Options</span></label></div></div><div className="flex items-center justify-between border-t border-line pt-5"><p className="text-[12px] text-ink-soft">Settings for <strong>{selectedExamId}</strong></p><button onClick={save} className="border border-forest bg-forest px-5 py-2.5 font-mono text-[10px] uppercase tracking-wider text-paper hover:bg-forest-light">{saved ? "Saved" : "Save security settings"}</button></div></div>;
}
