...
type Student = { roll: string; name: string; email: string; source: "manual" | "bulk" | "seed" };

const SEED: Student[] = [
  { roll: "21VGN0142", name: "B. Priya Nikitha", email: "21vgn0142@vignan.ac.in", source: "seed" },
  { roll: "21VGN0158", name: "K. Rohan Teja", email: "21vgn0158@vignan.ac.in", source: "seed" },
  { roll: "21VGN0163", name: "M. Sai Charan", email: "21vgn0163@vignan.ac.in", source: "seed" },
  { roll: "21VGN0171", name: "A. Deepika Reddy", email: "21vgn0171@vignan.ac.in", source: "seed" },
];
const BULK_SAMPLE: Student[] = [
  { roll: "21VGN0184", name: "S. Vamsi Krishna", email: "21vgn0184@vignan.ac.in", source: "bulk" },
  { roll: "21VGN0190", name: "T. Harika", email: "21vgn0190@vignan.ac.in", source: "bulk" },
  { roll: "21VGN0205", name: "N. Aravind", email: "21vgn0205@vignan.ac.in", source: "bulk" },
];
...
export default function TeacherStudents({ exams, navigate, notify }: { exams: Exam[]; navigate: (path: string) => void; notify: (message: string) => void }) {
  ...
  const [roster, setRoster] = useState<Student[]>(SEED);
  ...
  const addManual = () => {
    ...
    if (roster.some((r) => r.roll === roll || r.email === email)) {
      setError("A student with that roll number or email is already on the roster.");
      return;
    }
  ...
  const downloadTemplate = () => {
    ...
    const header = "roll_no,full_name,email,batch\n";
    const sample = `21VGN0999,Sample Student,21vgn0999@vignan.ac.in,${batch?.batch ?? ""}\n`;
    const blanks = ",,,\n".repeat(8);
  ...
  const onBulkStage = (file: File | undefined) => {
    ...
    const fresh = BULK_SAMPLE.filter((b) => !roster.some((r) => r.roll === b.roll));
    setRoster((cur) => [...fresh, ...cur]);
    notify(`${fresh.length} students imported from ${file.name}`);
  };
  ...
  return (
    <div className="space-y-6">
      <StuHeader batch={batch} exams={exams} onBatch={setBatch} counts={counts} onExams={() => navigate("/teacher/exams")} />
      ...
      <RosterTable visible={visible} total={roster.length} search={search} setSearch={setSearch} onRemove={remove} />
    </div>
  );
}

function StuHeader({ batch, exams, onBatch, counts, onExams }: { batch: Exam | null; exams: Exam[]; onBatch: (e: Exam | null) => void; counts: { total: number; manual: number; bulk: number }; onExams: () => void }) {
  return (
    <div className="border border-line bg-paper p-6 sm:p-7">
      ...
      <div className="mt-5 flex flex-wrap items-end gap-3 border-t border-line pt-5">
        ...
        <button onClick={onExams} className="ml-auto border border-line-strong px-4 py-2.5 font-mono text-[10px] uppercase tracking-wider text-ink-soft hover:border-forest hover:text-forest">
          Manage exams →
        </button>
      </div>
    </div>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
        <div className="flex flex-col justify-between border border-line bg-paper-raised p-5">
          <div>
            ...
            <p className="mt-2 text-[13px] text-ink-soft">Columns: <span className="font-mono text-[12px] text-ink">roll_no, full_name, email, batch</span></p>
          </div>
...
function RosterTable({ visible, total, search, setSearch, onRemove }: { visible: Student[]; total: number; search: string; setSearch: (v: string) => void; onRemove: (roll: string) => void }) {
  return (
    <section className="border border-line bg-paper">
