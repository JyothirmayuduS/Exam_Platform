...
import { PlusIcon, ArrowLeftIcon, ArrowRightIcon, NumberField } from "../components/ui";
...
export function PageHeading({ eyebrow, title, detail, action }: { eyebrow: string; title: string; detail: string; action?: React.ReactNode }) { return <div className="flex flex-col justify-between gap-4 border-b border-line pb-6 md:flex-row md:items-end"><div><p className="font-mono text-[10px] uppercase tracking-widest text-soft">{eyebrow}</p><h1 className="mt-1 font-serif text-3xl font-semibold tracking-tight">{title}</h1><p className="mt-2 text-[13px] text-soft">{detail}</p></div>{action && <div>{action}</div>}</div>; }
export function Button({ children, onClick, primary = false, disabled = false, icon, iconRight, className = "", title, size = "md", variant }: { children: React.ReactNode; onClick?: () => void; primary?: boolean; disabled?: boolean; icon?: React.ReactNode; iconRight?: React.ReactNode; className?: string; title?: string; size?: "sm" | "md" | "lg"; variant?: "primary" | "secondary" | "ghost" | "danger" | "outline" }) { const pad = size === "sm" ? "px-3 py-1.5 text-[9px]" : size === "lg" ? "px-5 py-3 text-[11px]" : "px-4 py-2.5 text-[10px]"; const kind = variant ?? (primary ? "primary" : "secondary"); const tone = disabled ? "cursor-not-allowed border-line bg-line/30 text-soft" : kind === "primary" ? "border-forest bg-forest text-paper shadow-sm hover:bg-forest-soft hover:shadow" : kind === "danger" ? "border-alert/70 bg-alert text-paper shadow-sm hover:bg-alert/90" : kind === "ghost" ? "border-transparent bg-transparent text-soft hover:bg-raised hover:text-ink" : kind === "outline" ? "border-line bg-transparent text-ink hover:border-forest hover:text-forest" : "border-line bg-raised text-ink hover:border-forest hover:text-forest"; return <button title={title} onClick={onClick} disabled={disabled} className={`inline-flex select-none items-center justify-center gap-2 border font-mono uppercase tracking-wider transition-all duration-150 focus-visible:ring-2 focus-visible:ring-forest/40 focus-visible:ring-offset-2 focus-visible:ring-offset-paper active:translate-y-px ${pad} ${tone} ${className}`}>{icon}{children}{iconRight}</button>; }
...
function AnswerReleaseControl({ notify, examId }: { notify: (s: string) => void; examId: string }) {
  ...
  return <section className="mt-6 border border-line bg-paper p-5 sm:p-6">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><p className="font-mono text-[10px] uppercase tracking-widest text-soft">Answer release</p><h3 className="mt-1 font-serif text-lg font-semibold">When can students see the correct answers?</h3><p className="mt-1 max-w-2xl text-[12px] text-soft">Objective and coding questions are always scored instantly — this only controls when candidates may view the answer key and their result.</p></div><span className={`whitespace-nowrap border px-3 py-2 font-mono text-[10px] uppercase tracking-wider ${revealed ? "border-success bg-success/5 text-success" : mode === "auto" ? "border-forest bg-success/5 text-forest" : "border-amber bg-amber/5 text-amber"}`}>{saving ? "Saving…" : revealed ? <><FiCheck className="inline" aria-hidden /> Answers released</> : mode === "auto" ? "Auto release on" : "Hidden · manual"}</span></div>
    <div className="mt-5 grid gap-3 sm:grid-cols-2">{modes.map((m) => <button key={m.key} onClick={() => { setMode(m.key); setRevealed(false); void persist({ release_mode: m.key }); }} className={`border p-4 text-left ${mode === m.key ? "border-forest bg-success/5" : "border-line hover:border-line"}`}><span className="flex items-center gap-2"><span className={`flex h-4 w-4 items-center justify-center rounded-none border ${mode === m.key ? "border-forest" : "border-line"}`}>{mode === m.key && <span className="h-2 w-2 rounded-none bg-forest"/>}</span><span className="text-[13px] font-medium">{m.title}</span></span><span className="mt-2 block text-[12px] text-soft">{m.detail}</span></button>)}</div>
    {mode === "auto" && <div className="mt-4 border-l-2 border-forest bg-success/5 px-4 py-3"><p className="font-mono text-[10px] uppercase tracking-wider text-forest">Release timing</p><div className="mt-2 flex flex-wrap gap-2">{([["submit", "As soon as each student submits"], ["close", "When the exam window closes"]] as const).map(([key, label]) => <button key={key} onClick={() => { setAutoWhen(key); void persist({ release_timing: key }); }} className={`border px-3 py-2 font-mono text-[10px] uppercase tracking-wider ${autoWhen === key ? "border-forest bg-paper text-forest" : "border-line text-soft hover:text-ink"}`}>{label}</button>)}</div><p className="mt-2 text-[11px] text-soft">Releasing while the exam is still live lets early finishers share the key — “when the exam window closes” is the safer default.</p></div>}
    {mode === "manual" && <div className="mt-4 flex flex-col justify-between gap-3 border-t border-line pt-4 sm:flex-row sm:items-center"><p className="text-[12px] text-soft">{revealed ? "Answers are now visible to candidates in their results view." : "Answers are hidden. Reveal them once grading and review are complete."}</p><button onClick={() => { setRevealed(true); void persist({ results_published: true }); }} disabled={revealed || saving} className="whitespace-nowrap border border-forest bg-forest px-4 py-2.5 font-mono text-[10px] uppercase tracking-wider text-paper enabled:hover:bg-forest-soft disabled:cursor-not-allowed disabled:border-line disabled:bg-line/30 disabled:text-soft">{revealed ? "✓ Answers released" : saving ? "Saving…" : "Reveal answers now"}</button></div>}
  </section>;
}
...
function SelectField({ label, options, value, onChange }: { label: string; options: string[]; value?: string; onChange?: (value: string) => void }) { return <label className="block text-[12px] text-soft">{label}<select value={value} onChange={(e) => onChange?.(e.target.value)} className="mt-1 block w-full border border-line bg-paper px-3 py-2.5 text-[13px] text-ink">{options.map((option) => <option key={option}>{option}</option>)}</select></label>; }
...
function Field({ label, value }: { label: string; value: string }) { return <label className="text-[12px] text-soft">{label}<input value={value} readOnly className="mt-1 block w-full border border-line bg-paper px-3 py-2 text-[13px] text-ink"/></label>; }
